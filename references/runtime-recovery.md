...
Use `--fresh-attempt` to intentionally reset execution while keeping failure history and cumulative counts. It is not a new theorem. To repair a theorem, edit the claim and routing files consistently, then run:

```bash
python3 scripts/proof_runtime.py revise-claim path/to/project --reason "Exact change and why it is needed"
```

Old attempts remain in append-only history; hard exclusions and active briefs use the current claim hash and revision. Unversioned historical route fingerprints are not trusted as new hard exclusions. With an older project that has no checkpoint, inspect its last packet before choosing a fresh action and supply useful artifacts as references. The controller does not infer a reliable pending action from old prose logs.

A stale reference cannot silently change a pending proof: provide its path again explicitly, then let the pending action recheck it. A changed candidate or a damaged checkpoint produces a runtime error rather than accepted evidence. Preserve the historical files, inspect the mismatch, and use an intentional fresh attempt or theorem repair as appropriate.
...
`needs-evidence` is a request to the proof owner, not an automatic external invocation. Its packet contains a local claim, assumptions, requested capability, missing artifact, acceptance test, candidate identity when available, and resume phase. A `tool-replay` request differs from literature retrieval; an uncertain model verdict is not a route refutation.
