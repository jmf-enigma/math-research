...
For an exact Python checker, use `--backend Python`, its actual version, `--command-json '["python3","tool_checks/L3.py"]'`, and `--input tool_checks/L3.py`. Standard or versioned Python executable paths are accepted alongside `codex-math-python`. Python must execute the recorded script directly; `-c` and `-m` are not script entrypoints. Virtual environments retain the invoked interpreter path, while the resolved binary remains fingerprinted.

The runner uses no shell, allows only named math backends, requires an installed executable outside the project, checks input and executable fingerprints before and after replay, applies a timeout with process-group cleanup, and records stdout and stderr. A changed input, expected answer, or claim revision cannot return a pass; a revised theorem never receives the old replay event. An artifact edited during replay is preserved and rejected. `audit` additionally checks that the artifact still lives in the current project, its hashes and latest replay files agree, and its latest passed replay has a matching runtime event. A copied JSON summary or stale ledger event is not a live artifact. It executes trusted proof-project scripts; it is an audit and replay boundary, not a sandbox for hostile code. Exact counterexamples, symbolic identities, condition sets, and solver certificates must match a canonical expected stdout; `exit-only` is retained only for numerical exploration, unclassified runs, or formal-tool process checks and never establishes a mathematical output by itself. `numerical-evidence` remains conjectural, and every symbolic, solver, or formal result remains a candidate artifact until translated and independently checked.

Within one project diagnosis, computation records are validated and indexed once. The index is discarded between diagnoses and rejected if its history file changes; current input, output, and executable fingerprints are still checked for each artifact.
